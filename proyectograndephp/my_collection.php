...
// Auth Check
// Comprobamos con $_SESSION que el usuario ha ingresado Y que tiene rol de 'coleccionista' o 'admin'
if (!isset($_SESSION['usuario_id']) || !in_array($_SESSION['usuario_rol'], ['coleccionista', 'admin'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // DELETE
    // Aquí empieza la parte de BORRAR (DELETE) del CRUD de colecciones personales
    if (isset($_POST['delete_id'])) {
        // Consulta SQL DELETE: Quita un juego de la colección personal del usuario (asegurando que sea su ID de usuario)
    ...
    // Aquí empieza la parte de CREAR (INSERT) o ACTUALIZAR (UPDATE)
    if ($collection_id) {
        // Consulta SQL UPDATE: Actualiza el estado físico y la nota de un juego que ya está en la colección 
        ...
        // Check duplicate
        // Consulta SQL SELECT: Busca comprobar si el usuario ya tenía previamente este juego agregado a su colección personal
        ...
        // Consulta SQL INSERT: Agrega un nuevo juego a la lista privada del usuario (colecciones)
...
// Edit Mode
// Comprobamos si recibimos una petición 'edit' por la URL para rellenar los datos en el formulario
if (isset($_GET['edit'])) {
    // Consulta SQL SELECT (READ para edición): Obtiene los detalles de estado y nota de la entrada existente
...
// Aquí empieza la parte principal de LEER (READ) del CRUD de colecciones
// Consulta SQL SELECT: Combina la tabla colecciones, videojuegos y sistemas (mediante JOIN) para 
// pintar todos los juegos que el usuario actual ($user_id) posee
$query = "
...
// Data for Dropdown
// Consulta SQL SELECT: Obtiene el listado COMPLETO de videojuegos para que el usuario pueda escoger cuál agregarse en el desplegable.
    ...
    while ($row = $mi_coleccion->fetch_assoc()):
        ...
        <div class="card">
            <div class="card-image-container"
                style="display: flex; align-items: center; justify-content: center; background-color: #1e293b; height: 150px; color: var(--neon-blue); font-size: 1.5rem; font-weight: bold; text-align: center;">
                [<?php echo htmlspecialchars($row['sistema']); ?>]
            </div>
